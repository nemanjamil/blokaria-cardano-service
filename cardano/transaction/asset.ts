...
export class AssetTransaction extends SimpleTransaction {
  ...
  build(): TxFile {
    ...
    const command = `${this.getCliPath()} transaction ${this.getEra()} build ${txInParam} --tx-out ${txOut}+${amountToSend}+"1 ${policyId}.${assetName}" --invalid-hereafter ${invalidHereAfter} --invalid-before ${invalidBefore} --change-address ${changeAddress} --${this.getNetwork()}${
